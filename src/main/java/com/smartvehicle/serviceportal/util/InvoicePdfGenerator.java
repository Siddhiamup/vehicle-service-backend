    ...
    public static byte[] generateInvoicePdf(Invoice invoice) {
        ...
        try {
            ...
            document.add(new Paragraph("Customer Name: "
                    + invoice.getBooking().getVehicle().getUser().getName()));
            document.add(new Paragraph("Email: "
                    + invoice.getBooking().getVehicle().getUser().getEmail()));
            document.add(new Paragraph(" "));

            // -------- VEHICLE DETAILS --------
            document.add(new Paragraph("Vehicle Number: "
                    + invoice.getBooking().getVehicle().getVehicleNumber()));
            document.add(new Paragraph("Model: "
                    + invoice.getBooking().getVehicle().getModel()));
            document.add(new Paragraph(" "));

            // -------- SERVICE DETAILS --------
            document.add(new Paragraph("Service: "
                    + invoice.getBooking().getService().getServiceName()));
            document.add(new Paragraph("Duration (hours): "
                    + invoice.getBooking().getService().getDurationHours()));
